...
It is interesting how undergraduate students' goals change through-out their studies. A formal analysis started with an interpretion of the 7 categories based on the questions they consisted of. The results are presented in the table below:
...
Because `Interest`, `Enjoyment` and `Importance focus` categories consisted only of 1 question, no further computation was required to interpret the data. For the remaining categories, which number of questions was equal to 3, a mean for each assessment category was computed and saved for each individual student. 
...
To test the hypotheses, This resulted in 4 extra columns added to the original data set, since Interest, Enjoyment and Importance category consisted only of 1 question. An example of the new columns and their contents can be seen below:

```{r}
dat_tibble[1:5, 20:23]
```

An exploratory data analysis using graphs was performed. Some of the worth mentioning are:
