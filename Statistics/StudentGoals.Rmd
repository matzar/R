...
| |
| - | - |
| `r tally(males)` | `r tally(females)` |

```{r}
d <- ggplot(filter(dat, sex == 'Male'), aes(filter(dat, sex == 'Male')$year, filter(dat, sex == 'Male')$MG))
# mapping data (use "jitter" to improve the graph and avoid gridding)
l <- d + geom_jitter(aes(colour = filter(dat, sex == 'Male')$subject))
# smoothing
s <- l + geom_smooth(method = stats::loess, formula = y ~ log(x), se = TRUE)
# adding labels
t <- s + labs(
...
# s + geom_jitter(filter(dat, sex == 'Female'), aes(filter(dat, sex == 'Female'), filter(dat, sex == 'Female')$year, filter(dat, sex == 'Female')$MG))

A = data.frame(x = filter(dat, sex == 'Female')$year, y=filter(dat, sex == 'Female')$MG)
B = data.frame(x = filter(dat, sex == 'Male')$year, y=filter(dat, sex == 'Male')$MG)
ggplot(A,aes(x,y)) + geom_smooth() + geom_jitter(data=B,colour='red') + geom_smooth()
